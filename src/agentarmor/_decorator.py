...
def _record_cost_report(
    ...
    workflow: Workflow | None,
    report: StepCostReport | None,
) -> tuple[float, AgentArmorBudgetExceededError | None]:
    if report is None:
        return 0.0, None
    ...
    except AgentArmorBudgetExceededError as exc:
        return cost_usd, exc
    ...
    def decorator(target: Callable[P, R]) -> Callable[P, R]:
        ...
        if asyncio.iscoroutinefunction(target):
            ...
            async def async_wrapper(*args: P.args, **kwargs: P.kwargs) -> Any:
                ...
                step_cost_usd = 0.0
                budget_error: AgentArmorBudgetExceededError | None = None
                try:
                    try:
                        if breaker is not None and not breaker.can_execute():
                            fallback = breaker.config.fallback
                            if fallback is None:
                                open_exc = AgentArmorCircuitOpenError(
                                    function_name,
                                    breaker.state,
                                )
