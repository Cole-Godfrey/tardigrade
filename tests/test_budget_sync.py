...
def test_sync_budget_degrade_calls_callback(tmp_path: Path) -> None:
    ...
    @armor(name="step")
    def step() -> str:
        return "done", StepCostReport(cost_usd=0.02, model="gpt-4o")
...
def test_sync_budget_warning_logs_at_fraction(tmp_path: Path) -> None:
    ...
    @armor(name="step")
    def step() -> str:
        return "done", StepCostReport(cost_usd=0.60, model="gpt-4o")
...
def test_sync_budget_extracts_cost_report_without_workflow() -> None:
    @armor(name="step")
    def step() -> str:
        return "value", StepCostReport(cost_usd=0.40, model="gpt-4o")
...
def test_sync_budget_tracks_fallback_cost(tmp_path: Path) -> None:
    ...
    def fallback(prompt: str) -> tuple[str, StepCostReport]:
        ...
        fallback_calls += 1
        return "fallback", StepCostReport(cost_usd=0.03, model="gpt-4o-mini")
    ...
    try:
        ...
        assert workflow.cost_tracker.step_costs[0]["model"] == "gpt-4o-mini"
    finally:
        store.close()
...
def test_sync_budget_exposes_workflow_cost_tracker_after_run(tmp_path: Path) -> None:
    ...
    @armor(name="step")
    def step() -> str:
        return "done", StepCostReport(cost_usd=0.02, model="gpt-4o-mini")
