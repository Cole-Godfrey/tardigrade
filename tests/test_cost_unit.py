...
def test_cost_tracker_calculates_cost_for_known_model() -> None:
    tracker = CostTracker()
    report = StepCostReport(input_tokens=1000, output_tokens=500, model="gpt-4o")
...
def test_cost_tracker_record_accumulates_history() -> None:
    ...
    tracker.record("step_1", StepCostReport(cost_usd=0.01, model="gpt-4o-mini"))
    tracker.record("step_2", StepCostReport(cost_usd=0.02, model="gpt-4o"))
